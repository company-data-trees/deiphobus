...
var Keyspace = function(raw) {
    ...
    raw.cf_defs.toArray().forEach(function(raw) {
        var comparator = raw.comparator_type.split('org.apache.cassandra.db.marshal.').pop();
        var composite = raw.comparator_type.match(/^org.apache.cassandra.db.marshal.CompositeType\((.*)\)$/);
        if (composite) {
            comparator = composite[1].split(',').map(function(type) {
                return type.trim().split('org.apache.cassandra.db.marshal.').pop();
            });
        }
        ...
        cf.key_alias = raw.key_alias ? Marshal.for(cf.key_type).from(raw.key_alias) : 'KEY';
