...
var format = require('ringo/utils/strings').format;
...
Connection.prototype.use = function(keyspace) {
    ...
    this.keyspace = new Keyspace(this.execute('describe_keyspace', keyspace));
}
...
Connection.prototype.cql = function() {
    ...
    if (result.type == CqlResultType.ROWS) {
        // TODO: the result doesn't include a schema in 0.8 API,
        // so we'll need a fallback (from a describe_keyspace?)
        var schema = new Schema(result.schema);
        var rows = result.rows.toArray().map(function(row) {
