...
var columnRange = function(options) {
    ...
    var start = (typeof options.start == 'undefined' || options.start === null) ? EMPTY : marshal.to(options.start);
    var end = (typeof options.end == 'undefined' || options.end === null) ? EMPTY : marshal.to(options.end);

    var predicate = new SlicePredicate();
    var range = new SliceRange(start, end, reversed, limit);
...
Selector.prototype.get = function(column_family, keys, options) {
    ...
    if (options.columns instanceof Array || options.column instanceof Array) {
        columns = specificColumns(options.columns || options.column, marshalName);
    } else if (typeof options.column == 'object' || typeof options.columns == 'object') {
        columns = columnRange(merge(options.column || options.columns, {type: marshalName}));
    } else if (options.column) {
        columns = specificColumns([options.column], marshalName);
    }
    ...
    var result = rows.map(function(raw) {
        var row = {
            ...
            columns: options.return_columns_as_array ? [] : {}
        };
        ...
        (new ScriptableList(raw.columns)).forEach(function(raw) {
            ...
            if (options.return_columns_as_array) row.columns.push(entry)
            else row.columns[nameString] = entry;
...
Selector.prototype.iterate = // deprecated
Selector.prototype.iterate_rows = function(column_family, options) {
...
Selector.prototype.iterate_columns = function(column_family, key, options) {
    var range = merge(options || {}, { start: null, end: null, limit: 100 })
    var more = true;

    if (range.limit < 2) range.limit = 2;

    while (more) {
        var row = this.get(column_family, key, {
            columns: range,
            return_columns_as_array: true
        })[0];

        if (row.columns.length >= range.limit) range.start = row.columns.pop().name;
        else more = false;

        for (var index = 0; index < row.columns.length; index++)
            yield(row.columns[index]);
    }
